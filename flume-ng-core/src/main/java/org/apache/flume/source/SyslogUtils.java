...
public class SyslogUtils {
  ...
  //增加百姓网的时间格式
  public static final String SYSLOG_TIMESTAMP_FORMAT_BX_EVENT = "yyyyMMdd HHmmss";
  public Format bxEventFormater = new SimpleDateFormat(SYSLOG_TIMESTAMP_FORMAT_BX_EVENT);
  ...
  public static final String SYSLOG_MSG_RFC3164_0 =
      "(?:\\<(\\d{1,3})\\>)" +
          "(?:(\\d)?\\s?)" + // version
          // stamp MMM d HH:ßmm:ss, single digit date has two spaces
  ...
  Event buildEvent() {
    try {
      ...
      if (isBadEvent) {
        //注释掉这个报警，因为百姓网的UDP请求并不是syslog格式的
  ...
  private void reset() {
    baos.reset();
    //因为BX的UDP数据并不是SYSLOG格式，所以mode判断没有用，
    m = Mode.START;
  ...
  public Event extractEvent(ChannelBuffer in) {
    ...
    */
    ...
    String _now_t = bxEventFormater.format(new Date());
